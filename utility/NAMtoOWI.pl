...
sub rotateAndFormat
{
	for my $t (0 .. $nRec{'time'}-1)
		...
		# #run awis_interp
		 `$scriptDir/utility/awip_lambert_interp.x $awipGridNumber 3 $outFile $ptFile rotatedNAM.txt velocity $velocityMultiplier`;
...
sub getGrib2
{
        ...
        if ( $namFormat eq "grib2" ) {
           `$scriptDir/utility/wgrib2 $grib2Files[0] -match PRMSL` =~ m/d=(\d+)/;
           $startTime = $1;
        }
        ...
        foreach my $file (@grib2Files) {
           ...
           if ( $namType eq "nowcast" ) {
              if ( $namFormat eq "grib2" ) {
                 `$scriptDir/utility/wgrib2 $file -match PRMSL` =~ m/d=(\d+)/;
                 $endTime = $1;
              } 
              if ( $namFormat eq "grb" ) {
           ...
           if ( $namFormat eq "grib2" ) { 
              ...
              @rawU = `$scriptDir/utility/wgrib2 $file -match "UGRD:10 m" -inv /dev/null -text -`;
              @rawV = `$scriptDir/utility/wgrib2 $file -match "VGRD:10 m" -inv /dev/null -text -`;
              @rawP = `$scriptDir/utility/wgrib2 $file -match "PRMSL:mean" -inv /dev/null -text -`;
           }
           if ( $namFormat eq "grb" ) {
              #
